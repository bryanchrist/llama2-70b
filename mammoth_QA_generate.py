...
adapter_path = "mammoth_QA_adapter/checkpoint-3500"  # Specify the path to the adapter weights
...
for i in range(0,100):
    ...
    formatted_prompt.append(f"Below is an instruction that describes a task. "
                f"Write a response that appropriately completes the request.\n\n"
                f"### Instruction:\nWrite a {grade} math word problem and Python program to solve the word problem.\n\n### Response: ")
    formatted_prompt = "\n\n".join(formatted_prompt)
    inputs = tokenizer.encode(formatted_prompt, return_tensors="pt")
    attention_mask = torch.ones_like(inputs)
    inputs = inputs.to('cuda')
