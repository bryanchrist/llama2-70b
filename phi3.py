import os
os.environ['TRANSFORMERS_CACHE'] = '/scratch/brc4cb/phi3/cache'
...
for i in range(0, len(df)-6):
    ...
    answer = df.iloc[i]['answer']
    final_prompt = f"""<|user|>\n{final_question} Let's write a Python program. <|end|>\n<|assistant|>"""
    
    for j in range(len(df)-5, len(df)):
        question = df['question'].iloc[j]
        solution = df['solution'].iloc[j]
        prompt = f"""<|user|>\n{question} Let's write a Python program. <|end|>\n<|assistant|>\n{solution}\n<|end|>"""
...
print(f"First attempt solve rate %: {np.mean(solved)*100}")
print(f"% exec code: {np.mean(exec_code)*100}")

output_file = "phi_test.txt"  # Specify the path and filename for the output file

with open(output_file, "a") as f:  # Open the file in append mode ("a")
    f.write(f"""-----------First attempt solve rate %: {np.mean(solved)*100}""" + "\n" + f"% exec code: {np.mean(exec_code)*100}" + "\n\n ----")  # Append the newly generated text to the file
